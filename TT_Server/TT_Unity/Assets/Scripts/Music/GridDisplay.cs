...
public class GridDisplay : MonoBehaviour
{
    ...
    public static void HitTokenUpdate(string vehicle, int tokenNum, TokenSpawner.Colours tokenColour)
    {
        //Debug.Log("TokenHit::" + vehicle + " with " + tokenNum);

        // TODO:: check that these are actually left or right (might be wrong way arround)
        VehicleTokenStatus token1;
        VehicleTokenStatus token2;

        HoverMotor.VehicleStates state;

        if (VehicleManager.IsPlayerById(vehicle))
        {
            token1 = Instance._trackEnabler[tokenNum];
            token2 = Instance._trackEnabler[(tokenNum + 1) % 2];

            token1.TokenHit(tokenColour, vehicle);
        }
        else
        {
            token1 = Instance._vehicleDictionary[vehicle][tokenNum];
            token2 = Instance._vehicleDictionary[vehicle][(tokenNum + 1) % 2];
        ...
        if (tokenNum == 1)
        {
            state = workOutState(token1, token2);
        }
        else
        {
            state = workOutState(token2, token1);
        }

        VehicleManager.SetVehicleState(vehicle, state);
    }

    private static HoverMotor.VehicleStates workOutState(VehicleTokenStatus token1, VehicleTokenStatus token2)
    {
        // TODO:: may need to swap Right and Left...
        int type = 0;
        ...
        if (type == 3 && token1.CurrColour != token2.CurrColour)
        {
            ++type;
        }
        ...
        return (HoverMotor.VehicleStates) type;
    }
    ...
    public class VehicleTokenStatus
    {
        ...
        public void TokenHit(TokenSpawner.Colours tokenColour, string VehicleID)
        {
            vehicleID = VehicleID;
        ...
        public void beatSectionUpdate()
        {
            ...
            if (CountSlider != null)
            {
                ...
                else if (Counter == 0)
                {
                    ...
                    VehicleManager.SetVehicleState(vehicleID, HoverMotor.VehicleStates.NONE);
