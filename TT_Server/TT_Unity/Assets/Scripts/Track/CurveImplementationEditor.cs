...
[ExecuteInEditMode()]
public class CurveImplementationEditor : MonoBehaviour
{
    public List<Waypoint> _points = new List<Waypoint>();

    public int CurveResolution = 200;
    ...
    private static CurveImplementationEditor _curve;

    // Use the static object pattern to guarantee that this object is correctly assigned and pressent in the scene.
    public static CurveImplementationEditor Instance
    {
        get
        {
            if (!_curve)
            {
                _curve = FindObjectOfType(typeof(CurveImplementationEditor)) as CurveImplementationEditor;
